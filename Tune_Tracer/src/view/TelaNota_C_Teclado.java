...
public class TelaNota_C_Teclado extends JFrame {
	...
	ImageIcon C = new ImageIcon(getClass().getResource("C_Teclado.jpeg"));
	...
	public TelaNota_C_Teclado() {
		...
		imagemC.setBounds(233, 126, 306, 195);
	...
	private void favoritarNOTA() {
		...
		try (Connection conected = sq.getConect()) {
			FavoritosVO FV = new FavoritosVO("C", "Teclado");
	...
	private void DesfavoritarNOTA() {
		...
		try (Connection conected = sq.getConect()) {
			FavoritosVO si = new FavoritosVO("C", "Teclado");
			FavoritosInsertDAO FID = new FavoritosInsertDAO(conected);
			FID.desfavoritar(si);
